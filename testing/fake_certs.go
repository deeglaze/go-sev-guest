...
func (s *AmdSigner) CertTableBytes() ([]byte, error) {
	...
	for guid, data := range s.Extras {
		prior := index
		index++
		headers[index].GUID = abi.MustParseGUID(guid)
